...
SELECT participant_id, name, address 
FROM Participants
...
VALUES (#age_group_description_dropdown_input, concat(#fname, #lname) AS #name, #address);

--- Update Existing Participants ---
UPDATE Participants 
    SET 
        name = concat(#fname, #lname) AS #name,
...
SELECT volunteer_id, name, email, role 
FROM Volunteers
ORDER BY participant_id ASC;

--- Create New Volunteers ---
INSERT INTO Volunteers (name, email, role)
VALUES (#name, #email, #role_dropdown_input);
...
SELECT age_group_id, description 
FROM Age_Groups
...
UPDATE Activities
    SET
        Location = #location_name_dropdown_input,
